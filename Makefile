...
# 包含路径 (请根据您的环境修改)
# pkg-config 会自动处理 FFmpeg 和 freetype2 的路径。
# 我们只需保留 rga 和当前目录的路径。
INCLUDES = -I. \
           -I/usr/include/rga
...
# -Wall: 开启所有常用警告
# -O2: 优化级别
# -fPIC: 生成位置无关代码，这是创建库所必需的
# -D_REENTRANT: 为多线程程序定义预处理器宏
CXXFLAGS = -std=c++14 -Wall -O2 -fPIC -D_REENTRANT $(INCLUDES) $(PKG_CFLAGS)
...
# --- SDK 库定义 ---
# [更新] 添加了 file_manager.cpp 和 file_utils.cpp
# 组成静态库的所有源文件 (除了示例 main.cpp)
SDK_SOURCES = camera_sdk.cpp camera_controller.cpp \
			  camera_capture.cpp \
			  recorder.cpp snapshotter.cpp rtsp_streamer.cpp \
			  osd_manager.cpp zoom_manager.cpp exposure_manager.cpp \
			  file_manager.cpp file_utils.cpp
...
$(SDK_TARGET): $(SDK_OBJECTS)
	...
	@mkdir -p $(LIB_DIR)
	$(AR) rcs $@ $^
	@echo "===> 静态库构建完成: $(SDK_TARGET)"

# 通用编译规则：如何从 .cpp 文件生成 .o 文件
# $@: 规则的目标文件名 (e.g., build/obj/camera_controller.o)
# $<: 规则的第一个依赖文件名 (e.g., camera_controller.cpp)
# $(@D): 目标文件的目录部分 (e.g., build/obj)
$(OBJ_DIR)/%.o: %.cpp
	@echo "===> 编译: $< -> $@"
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -c $< -o $@
